    ...
    const struct MAPDRAW_OPTIONS *opts)
{
    ...
    if (draw_data==NULL)
    {
        message_error("generate_map_bitmap: Cannot allocate draw_data memory.");
        return false;
    }
    ...
    {
        message_error("generate_map_bitmap: Out of memory.");
        return false;
    }
    ...
    {
        message_error("generate_map_bitmap: Cannot allocate palette memory.");
        return false;
    }
    ...
    draw_data->rescale=(opts->rescale);
    ...
    if (result)
    {
      message_log(" generate_map_bitmap: Loading \"%s\"",palette_fname);
      fnames=NULL;
    ...
    if (result)
    {
      message_log(" generate_map_bitmap: Loading \"%s\"",cube_fname);
      fnames=NULL;
    ...
    if (result)
    {
      message_log(" generate_map_bitmap: Loading \"%s\"",tmapanim_fname);
      fnames=NULL;
    ...
    if (result)
    {
      message_log(" generate_map_bitmap: Loading texture");
      fnames=NULL;
      format_data_fname(&fnames,opts->data_path,"tmapa%03d.dat",(int)(lvl->inf%8));
    ...
    if (result)
    {
      message_log(" generate_map_bitmap: Loading gui2-0 icons");
      char *tabfname;
      fnames=NULL;
      tabfname=NULL;
