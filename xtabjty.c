...
short create_images_jtytab_idx(struct IMAGELIST *images,const char *jtyfname,const char *tabfname,int verbose)
{
    ...
    struct DATFILE jtyf;
    {
        ...
        case JTYTB_CANNOT_OPEN:
             if (verbose) printf("\nError - Cannot open JTY file: %s\n",jtyfname);
             return JTYTB_CANNOT_OPEN;
        default:
             if (verbose) printf("\nError - Loading JTY file %s returned fail code %d\n",jtyfname,retcode);
             return retcode;
        }
    }
    if (verbose) printf("Done.\n");
...
short read_jtytabfile_data(struct JTYTABFILE *jtabf,const char *srcfname)
{
    ...
    for (entrynum=0;entrynum<jtabf->count;entrynum++)
    {
            ...
            memcpy(curitm->unkn,tabitm+6,10);
...
int read_jtytab_images(struct IMAGELIST *images,unsigned long *readcount,struct JTYTABFILE *jtabf,struct DATFILE *jtyf,const int verbose)
{
